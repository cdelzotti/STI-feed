"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.OutputController = void 0;
const common_1 = require("@nestjs/common");
const output_pipe_1 = require("./output.pipe");
const output_service_1 = require("./output.service");
let OutputController = class OutputController {
    constructor(outputService) {
        this.outputService = outputService;
    }
    async getMsg(body) {
        return this.outputService.getMessages(body);
    }
};
__decorate([
    common_1.Post("msg/"),
    __param(0, common_1.Body(new output_pipe_1.MessageRequest())),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], OutputController.prototype, "getMsg", null);
OutputController = __decorate([
    common_1.Controller("output/"),
    __metadata("design:paramtypes", [output_service_1.OutputService])
], OutputController);
exports.OutputController = OutputController;
//# sourceMappingURL=output.controller.js.map